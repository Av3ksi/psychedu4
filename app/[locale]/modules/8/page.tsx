...
// --- DATENSTRUKTUR FÜR MODUL 8 ---
// Modul 8 hat 7 Lektionen. Titel/Beschreibungen kommen aus JSON.
// Relevanz-Werte sind Platzhalter und können angepasst werden.
const personalityPsychologyModulesBase = [
...
// --- HAUPTKOMPONENTE FÜR DIE ÜBERSICHTSSEITE (ANGEPASST FÜR MODUL 8) ---
export default function PersonalityPsychologyPage() {
    ...
    // Neue Namespaces für die Inhalte von Modul 8
    const tPage = useTranslations('personalityPsychologyOverview');
    const tTitles = useTranslations('personalityPsychologyModuleTitles'); 
    const tDesc = useTranslations('personalityPsychologyModuleDescriptions'); 
    ...
    const requiresPremium = true; // Modul 8 ist Premium
    ...
    return (
        <div className="flex">
            ...
            <aside className="sticky top-20 h-[calc(100vh-5rem)] w-64 flex-shrink-0 p-4 hidden lg:block overflow-y-auto">
                ...
                {/* Neuer Key für Modul 8 Sidebar-Titel */}
                <h2 className="font-bold mb-4">{tModules('personalitySidebarTitle')}</h2>
                <nav className="space-y-2">
                    {personalityPsychologyModulesBase.map(module => (
                        <ScrollLink 
                            key={module.id} 
                            to={`module-${module.id}`} 
                            spy={true} 
                        ...
                        >
                            ...
                            {/* Holt Titel aus 'personalityPsychologyModuleTitles' */}
            ...
            <main className="flex-grow p-6 md:p-8">
                <div className="max-w-4xl mx-auto">
                    {/* Verwendet Titel und Beschreibung aus 'personalityPsychologyOverview' */}
                        ...
                        {personalityPsychologyModulesBase.map(module => (
                            <Element key={module.id} name={`module-${module.id}`}>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <SectionCard title={tModules('cardTitleGrundwissen')} icon={<BookOpen className="w-6 h-6 text-primary"/>} href={`/modules/8/grundwissen-${module.id}`}>
                                        <p>{tDesc(`${module.id}_grundwissen`)}</p>
                                    </SectionCard>
                                    <SectionCard title={tModules('cardTitleAnwendbarkeit')} icon={<Target className="w-6 h-6 text-primary"/>} href={`/modules/8/anwendbarkeit-${module.id}`}>
                                         <p>{tDesc(`${module.id}_anwendbarkeit`)}</p>
                                    </SectionCard>
                                    <SectionCard title={tModules('cardTitleMeisterklasse')} icon={<BrainCircuit className="w-6 h-6 text-primary"/>} href={`/modules/8/meisterklasse-${module.id}`}>
                                        <p>{tDesc(`${module.id}_meisterklasse`)}</p>
                                    </SectionCard>
                                    <SectionCard title={tModules('cardTitleUebungen')} icon={<Lightbulb className="w-6 h-6 text-primary"/>} href={`/modules/8/uebungen-${module.id}`}>
