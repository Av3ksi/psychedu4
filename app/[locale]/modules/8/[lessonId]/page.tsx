...
const ToggleSolution: FC<{ question: OpenQuestion }> = ({ question }) => {
    ...
    // !! Namespace für Modul 8 UI !!
    const t = useTranslations('module8.ui');
    return (
...
const UebungenContent: FC<{ data: UebungenData, moduleNumber: number }> = ({ data, moduleNumber }) => {
    ...
    // !! Namespace dynamisch für Modul 8 UI !!
...
export default function LessonDetailPage() {
  ...
  let moduleIdStr: string | null = null; // Das ist die Lektionsnummer als String (z.B. '1', '7')
  let isValidId = false;
  if (lessonId && typeof lessonId === 'string' && lessonId.includes('-')) {
    ...
    // Prüfe, ob es Modul 8 ist und die Lektions-ID gültig ist (1-7)
    if (!isNaN(parsedModuleId) && parsedModuleId >= 1 && parsedModuleId <= 7) {
      moduleNumber = 8; // Fest auf Modul 8 setzen
      isValidId = true;
    }
  }

  // --- RUFE DEN MODUL-SPEZIFISCHEN HOOK JETZT HIER OBEN AUF ---
  // Verwende 'module8' oder einen Fallback
  const tModule = useTranslations(isValidId ? `module8` : 'module1'); // 'module1' als Fallback


  // --- 2. JETZT DIE PRÜFUNGEN UND FRÜHEN RETURNS ---
  if (!isValidId) {
    // Ungültige Lektions-ID für Modul 8
     return (
  ...
  // Wir verwenden jetzt moduleIdStr (der String '1' bis '7') als Key in den JSON-Daten
  ...
  if (!contentAvailable) {
      ...
      return (
          <div className="max-w-4xl mx-auto p-6 md:p-8">
              ...
              {content}
              {/* Link zurück zur Modul 8 Übersicht */}
              <Link href="/modules/8" className="flex items-center gap-2 text-primary hover:underline mt-6">
                  <ArrowLeft className="w-5 h-5" />
                  <span>{tModule('backToOverview')}</span>
              </Link>
  ...
  const prevLink = prevPart ? `/modules/8/${prevPart}-${lessonKey}` : null;
  const nextLink = nextPart ? `/modules/8/${nextPart}-${lessonKey}` : null;
