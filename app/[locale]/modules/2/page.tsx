...
// --- DATENSTRUKTUR (OHNE TITEL/BESCHREIBUNGEN) ---
// Titel und Beschreibungen kommen jetzt aus den JSON-Dateien
const socialPsychologyModulesBase = [
...
export default function SocialPsychologyPage() {
    ...
    // Neue Namespaces für die Inhalte von Modul 2
    const tTitles = useTranslations('socialPsychologyModuleTitles'); 
    const tDesc = useTranslations('socialPsychologyModuleDescriptions'); 
    ...
    const requiresPremium = true; // Modul 2 ist Premium
    ...
    // --- ENDE NEUE PREMIUM-CHECK LOGIK ---

    // --- BESTEHENDER CODE (JETZT ÜBERSETZBAR) ---
    // Wird nur angezeigt, wenn der Nutzer Premium hat
    return (
        <div className="flex">
            ...
            <aside className="sticky top-20 h-[calc(100vh-5rem)] w-64 flex-shrink-0 p-4 hidden lg:block overflow-y-auto">
                ...
                {/* Verwendet 'socialSidebarTitle' aus 'modulesOverview' */}
                <h2 className="font-bold mb-4">{tModules('socialSidebarTitle')}</h2>
                <nav className="space-y-2">
                    {socialPsychologyModulesBase.map(module => (
                        <ScrollLink 
                            key={module.id} 
                            to={`module-${module.id}`} 
                            spy={true} 
                        ...
                        >
                            ...
                            {/* Holt Titel aus 'socialPsychologyModuleTitles' */}
            ...
            <main className="flex-grow p-6 md:p-8">
                <div className="max-w-4xl mx-auto">
                    {/* Verwendet 'socialTitle' und 'socialDescription' aus 'modulesOverview' */}
                    <h1 className="text-4xl font-bold mb-4">{tModules('socialTitle')}</h1>
                    <p className="text-lg text-slate-600 dark:text-slate-400 mb-12">
                        {tModules('socialDescription')}
                    </p>
                        ...
                        {socialPsychologyModulesBase.map(module => (
                            <Element key={module.id} name={`module-${module.id}`}>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <SectionCard title={tModules('cardTitleGrundwissen')} icon={<BookOpen className="w-6 h-6 text-primary"/>} href={`/modules/2/grundwissen-${module.id}`}>
                                        <p>{tDesc(`${module.id}_grundwissen`)}</p>
                                    </SectionCard>
                                    <SectionCard title={tModules('cardTitleAnwendbarkeit')} icon={<Target className="w-6 h-6 text-primary"/>} href={`/modules/2/anwendbarkeit-${module.id}`}>
                                         <p>{tDesc(`${module.id}_anwendbarkeit`)}</p>
                                    </SectionCard>
                                    <SectionCard title={tModules('cardTitleMeisterklasse')} icon={<BrainCircuit className="w-6 h-6 text-primary"/>} href={`/modules/2/meisterklasse-${module.id}`}>
                                        <p>{tDesc(`${module.id}_meisterklasse`)}</p>
                                    </SectionCard>
                                    <SectionCard title={tModules('cardTitleUebungen')} icon={<Lightbulb className="w-6 h-6 text-primary"/>} href={`/modules/2/uebungen-${module.id}`}>
