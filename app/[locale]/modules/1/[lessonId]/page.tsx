...
const ToggleSolution: FC<{ question: OpenQuestion }> = ({ question }) => {
  ...
  const t = useTranslations('module1.ui');
  return (
...
export default function LessonDetailPage() {
  ...
  if (lessonId && typeof lessonId === 'string' && lessonId.includes('-')) {
    ...
    if (!isNaN(parsedModuleId) && parsedModuleId >= 1 && parsedModuleId <= 16) {
      moduleNumber = 1;
      isValidId = true;
    }
  }

  const tModule = useTranslations(isValidId ? `module1` : 'module1');
  ...
  const prevLink = prevPart ? `/modules/1/${prevPart}-${lessonKey}` : null;
  const nextLink = nextPart ? `/modules/1/${nextPart}-${lessonKey}` : null;
