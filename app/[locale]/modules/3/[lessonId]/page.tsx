...
const ToggleSolution: FC<{ question: OpenQuestion }> = ({ question }) => {
  ...
  const t = useTranslations('module3.ui');
...
export default function LessonDetailPage() {
  ...
  if (lessonId && typeof lessonId === 'string' && lessonId.includes('-')) {
    ...
    if (!isNaN(parsedModuleId) && parsedModuleId >= 1 && parsedModuleId <= 9) {
      moduleNumber = 3;
      isValidId = true;
    }
  }

  const tModule = useTranslations(isValidId ? `module3` : 'module1');
  ...
  const prevLink = prevPart ? `/modules/3/${prevPart}-${lessonKey}` : null;
  const nextLink = nextPart ? `/modules/3/${nextPart}-${lessonKey}` : null;
