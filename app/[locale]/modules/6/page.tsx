...
const emotionMotivationModulesBase = [
...
export default function EmotionMotivationPage() {
    ...
    // Neue Namespaces für die Inhalte von Modul 6
    const tPage = useTranslations('emotionMotivationOverview');
    const tTitles = useTranslations('emotionMotivationModuleTitles'); 
    const tDesc = useTranslations('emotionMotivationModuleDescriptions'); 
    ...
    return (
        <div className="flex">
            ...
            <aside className="sticky top-20 h-[calc(100vh-5rem)] w-64 flex-shrink-0 p-4 hidden lg:block overflow-y-auto">
                ...
                {/* Verwendet 'emotionMotivationSidebarTitle' aus 'modulesOverview' */}
                <h2 className="font-bold mb-4">{tModules('emotionMotivationSidebarTitle')}</h2>
                <nav className="space-y-2">
                    {emotionMotivationModulesBase.map(module => (
                        <ScrollLink 
                            key={module.id} 
                            to={`module-${module.id}`} 
                            spy={true} 
                        ...
                        >
                            ...
                            {/* Holt Titel aus 'emotionMotivationModuleTitles' */}
            ...
            <main className="flex-grow p-6 md:p-8">
                <div className="max-w-4xl mx-auto">
                    {/* Verwendet Titel und Beschreibung aus 'emotionMotivationOverview' */}
                        ...
                        {emotionMotivationModulesBase.map(module => (
                            <Element key={module.id} name={`module-${module.id}`}>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <SectionCard title={tModules('cardTitleGrundwissen')} icon={<BookOpen className="w-6 h-6 text-primary"/>} href={`/modules/6/grundwissen-${module.id}`}>
                                        <p>{tDesc(`${module.id}_grundwissen`)}</p>
                                    </SectionCard>
                                    <SectionCard title={tModules('cardTitleAnwendbarkeit')} icon={<Target className="w-6 h-6 text-primary"/>} href={`/modules/6/anwendbarkeit-${module.id}`}>
                                         <p>{tDesc(`${module.id}_anwendbarkeit`)}</p>
                                    </SectionCard>
                                    <SectionCard title={tModules('cardTitleMeisterklasse')} icon={<BrainCircuit className="w-6 h-6 text-primary"/>} href={`/modules/6/meisterklasse-${module.id}`}>
                                        <p>{tDesc(`${module.id}_meisterklasse`)}</p>
                                    </SectionCard>
                                    <SectionCard title={tModules('cardTitleUebungen')} icon={<Lightbulb className="w-6 h-6 text-primary"/>} href={`/modules/6/uebungen-${module.id}`}>
