...
const ToggleSolution: FC<{ question: OpenQuestion }> = ({ question }) => {
    ...
    // !! Namespace für Modul 7 UI !!
    const t = useTranslations('module7.ui');
    return (
...
const UebungenContent: FC<{ data: UebungenData, moduleNumber: number }> = ({ data, moduleNumber }) => {
    ...
    // !! Namespace dynamisch für Modul 7 UI !!
...
export default function LessonDetailPage() {
  ...
  if (lessonId && typeof lessonId === 'string' && lessonId.includes('-')) {
    ...
    // Prüfe, ob es Modul 7 ist und die ID gültig ist
    if (!isNaN(parsedModuleId) && parsedModuleId >= 1 && parsedModuleId <= 5) { // Modul 7 hat 5 Lektionen
      moduleNumber = 7; // Fest auf Modul 7 setzen
      isValidId = true;
    }
  }

  // --- RUFE DEN MODUL-SPEZIFISCHEN HOOK JETZT HIER OBEN AUF ---
  // Verwende 'module7' oder einen Fallback
  const tModule = useTranslations(isValidId ? `module7` : 'module1'); // 'module1' als Fallback

  // --- 2. JETZT DIE PRÜFUNGEN UND FRÜHEN RETURNS ---
  if (!isValidId) {
    // Ungültige Lektions-ID für Modul 7
     return (
  ...
  // Wir verwenden jetzt moduleIdStr (der String '1' bis '5') als Key in den JSON-Daten
  ...
  // Link verwendet jetzt lessonKey (String '1'-'5')
  const prevLink = prevPart ? `/modules/7/${prevPart}-${lessonKey}` : null;
  const nextLink = nextPart ? `/modules/7/${nextPart}-${lessonKey}` : null;
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 md:p-8">
      <div className="mb-8">
        {/* Link zurück zur Modul 7 Übersicht */}
