...
const ToggleSolution: FC<{ question: OpenQuestion }> = ({ question }) => {
    ...
    // !! Namespace für Modul 9 UI !!
    const t = useTranslations('module9.ui');
    return (
...
const UebungenContent: FC<{ data: UebungenData, moduleNumber: number }> = ({ data, moduleNumber }) => {
    ...
    // !! Namespace dynamisch für Modul 9 UI !!
    const t = useTranslations(`module${moduleNumber}.ui`);

    if (!data || !data.quiz || !data.open || !data.kreativ) {
        // Fallback, falls Übungsdaten fehlen
         return <p>{t('loadingError')}</p>; // Zeigt eine generische Fehlermeldung
    }
...
export default function LessonDetailPage() {
  ...
  if (lessonId && typeof lessonId === 'string' && lessonId.includes('-')) {
    ...
    // Prüfe, ob es Modul 9 ist und die Lektions-ID gültig ist (1-8)
    if (!isNaN(parsedModuleId) && parsedModuleId >= 1 && parsedModuleId <= 8) {
      moduleNumber = 9; // Fest auf Modul 9 setzen
      isValidId = true;
    }
  }

  // --- RUFE DEN MODUL-SPEZIFISCHEN HOOK JETZT HIER OBEN AUF ---
  // Verwende 'module9' oder einen Fallback
  const tModule = useTranslations(isValidId ? `module9` : 'module1'); // 'module1' als Fallback


  // --- 2. JETZT DIE PRÜFUNGEN UND FRÜHEN RETURNS ---
  if (!isValidId) {
    // Ungültige Lektions-ID für Modul 9
     return (
  ...
  try {
    ...
    if (contentKey === 'uebungen') {
      ...
      if (!uebungenData?.quiz || !uebungenData?.open || !uebungenData?.kreativ) throw new Error('Übungsdaten unvollständig für Lektion ' + lessonKey);
      ...
      if (!htmlContent) throw new Error('HTML-Inhalt nicht gefunden für Lektion ' + lessonKey + ', Typ ' + contentKey);
  ...
  const prevLink = prevPart ? `/modules/9/${prevPart}-${lessonKey}` : null;
  const nextLink = nextPart ? `/modules/9/${nextPart}-${lessonKey}` : null;
  ...
  const displayTitle = `${pageTypeFormatted}: ${lessonTitle}`;


  return (
    <div className="max-w-4xl mx-auto p-6 md:p-8">
      <div className="mb-8">
        {/* Link zurück zur Modul 9 Übersicht */}
