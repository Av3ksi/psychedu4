...
const ToggleSolution: FC<{ question: OpenQuestion }> = ({ question }) => {
  ...
  const t = useTranslations('module5.ui');
  return (
...
export default function LessonDetailPage() {
  ...
  const tModule = useTranslations(isValidId ? `module5` : 'module1');
  ...
  const prevLink = prevPart ? `/modules/5/${prevPart}-${lessonKey}` : null;
  const nextLink = nextPart ? `/modules/5/${nextPart}-${lessonKey}` : null;
