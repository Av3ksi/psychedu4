...
import { useSubscription } from '@/hooks/useSubscription'; // Importieren
import { Lock } from 'lucide-react'; // Icon für gesperrte Module

export default function ModulesPage() {
  const [items, setItems] = useState<{id:number; title:string; sort_order:number; done:boolean}[]>([])
  const [loading, setLoading] = useState(true)
  const { subscription, isLoading: isSubLoading } = useSubscription(); // Abo-Status abrufen
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 space-y-3">
      {items.map((m, index) => {
        const isLocked = index > 0 && !isPremium; // Modul 1 (index 0) ist frei, der Rest ist für Premium
